...
/// Следующий эл-нт последовательности Z_n = Z_{n-1} + C
...
/// Вычисляем последовательность комплексных чисел, пока число не выйдет
/// за границы по модулю
...
void render_p_task(Image& imageBuffer, Vector2& constant, float scale, Vector2& offset, int iterationCount, int numTreads = 1) {
    // Выводим значение константы
    DrawText((std::to_string(constant.x) + " " + std::to_string(constant.y)).c_str(), 10, 10, 20, LIGHTGRAY);
    // --------------------------
    std::vector<std::vector<int>> arr(imageBuffer.width, std::vector<int>(imageBuffer.height));
    auto begin = std::chrono::steady_clock::now();
#pragma omp task shared(arr, scale, offset, constant, imageBuffer, iterationCount) default(none)
    {
        for (int x = 0; x < imageBuffer.width; ++x) {
            for (int y = 0; y < imageBuffer.height; ++y) {
                const float px = float(x - imageBuffer.width / 2) * scale + offset.x;
